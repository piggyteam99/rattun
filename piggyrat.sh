...
iran_server_configuration() {  
	...
	# Prompt for Ports (Supports Range and Comma separated)
	echo -ne "[*] Enter your target ports separated by commas or ranges (e.g. 80,443,2000-2050): "
	read -r input_ports
	input_ports=$(echo "$input_ports" | tr -d ' ')
    
	declare -a raw_ports
    IFS=',' read -r -a parts <<< "$input_ports"
    for part in "${parts[@]}"; do
        if [[ "$part" == *-* ]]; then
            IFS='-' read -r -a range <<< "$part"
            start=${range[0]}
            end=${range[1]}
	...
	for port in "${raw_ports[@]}"; do
		if [[ "$port" =~ ^[0-9]+$ ]] && [ "$port" -gt 22 ] && [ "$port" -le 65535 ]; then
			if check_port "$port" "$transport"; then
			    colorize red "[ERROR] Port $port is in use and will be skipped."
			else
			    config_ports+=("$port")
			fi
		else
			colorize red "[ERROR] Port $port is Invalid. Skipped."
		fi
	done

	if [ ${#config_ports[@]} -eq 0 ]; then
		colorize red "No valid ports were entered. Exiting." bold
		sleep 2
		return 1
	else
        colorize green "[INFO] ${#config_ports[@]} ports successfully added to your configs."
    fi
...
EOF
    ...
    colorize green "All ${#config_ports[@]} ports added to your config successfully" done
    sleep 1
